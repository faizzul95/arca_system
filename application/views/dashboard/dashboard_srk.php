			...
			<div class="col-xl-12">
				...
				<div class="row">
					<div class="col-xl-6 col-md-12">
						<div class="card card-height-100">
							<div class="card-header align-items-center d-flex">
								<h4 class="card-title mb-0 flex-grow-1">Report by Scrutinize Status</h4>
							</div><!-- end card header -->
							<div class="card-body">
								<div id="chartScrutinize" data-colors='["--vz-danger", "--vz-info", "--vz-warning"]' class="apex-charts" dir="ltr"></div>
					...
					<div class="col-xl-6 col-md-12">
						<div class="card card-height-100">
							<div class="card-header align-items-center d-flex">
								<h4 class="card-title mb-0 flex-grow-1">Report by Application Status</h4>
							</div>
							...
							<div class="card-body">
								...
								<div class="row align-items-center">
									<div class="col-6">
										...
										<h4 class="mb-0" id="totalCountAllApplication">0</h4>
								...
								<div class="mt-4 pt-2">
									<div class="progress progress-lg rounded-pill">
										<!-- style="width: 0%" -->
										<div id="applyCountProgress" class="progress-bar bg-success" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
										<div id="notApplyCountProgress" class="progress-bar bg-warning" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
									</div>
								</div><!-- end -->
								...
								<div class="mt-2 pt-2">
									<div class="d-flex mb-2 mt-4">
										...
										<div class="flex-shrink-0">
											<p id="totalStudentVal" class="mb-0">0</p>
										</div>
									</div><!-- end -->
									<div class="d-flex mb-2">
										...
										<div class="flex-shrink-0">
											<p id="applyCountVal" class="mb-0">0</p>
										</div>
									</div><!-- end -->
									<div class="d-flex">
										<div class="flex-grow-1">
										...
										<div class="flex-shrink-0">
											<p id="notApplyCountVal" class="mb-0">0</p>
										</div>
									</div><!-- end -->
								</div><!-- end -->
...
<script>
	...
	async function getDataDashboard() {
		...
		const res = await callApi('get', 'dashboard/srk-data');
		...
		if (isSuccess(res)) {
			...
			$('#chartPending').text(data.chartScrutinize.Pending);
			$('#chartOffered').text(data.chartScrutinize.Offered);
			$('#chartReject').text(data.chartScrutinize.Reject);

			$('#totalCountAllApplication').text(data.reportStatus.totalApplication);
			$('#previousDataDiv').html(data.reportStatus.previousData);

			$('#applyCountProgress').attr('aria-valuenow', data.reportStatus.totalApply.percentage);
			$('#notApplyCountProgress').attr('aria-valuenow', data.reportStatus.totalNotApply.percentage);

			$('#applyCountProgress').css('width', data.reportStatus.totalApply.percentage + '%');
			$('#notApplyCountProgress').css('width', data.reportStatus.totalNotApply.percentage + '%');

			$('#totalStudentVal').text(data.reportStatus.totalStudent);
			$('#applyCountVal').text(data.reportStatus.totalApply.total);
			$('#notApplyCountVal').text(data.reportStatus.totalNotApply.total);

			$('#activityEvent').html(data.activity);

			chartDonut(data.chartScrutinize);
	...
	function chartDonut(data) {
		var options = {
			series: [data.Pending, data.Offered, data.Reject],
			labels: ["Pending", "Offered", "Unoffered"],
			chart: {
				type: "donut",
				height: 219
			},
		...
		var chart = new ApexCharts(document.querySelector("#chartScrutinize"), options);
		chart.render();
	}
