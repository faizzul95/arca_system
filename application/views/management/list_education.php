	...
	<div class="col-xl-12 mb-4">
		<div class="card ribbon-box border shadow-none mb-lg-0" id="bodyDiv">
			<div class="card-header text-muted">
				<span class="ribbon ribbon-primary ribbon-shape"><span> Education </span></span>
...
<script type="text/javascript">
	...
	async function getDataList() {
		...
		generateDatatable('dataList', 'serverside', 'management/list-education', 'nodatadiv', null,
			[{
				"width": "60%",
				"targets": 0
			}, {
	...
	function formModal(type = 'create', data = null) {
		const modalTitle = (type == 'create') ? 'REGISTER EDUCATION LEVEL' : 'UPDATE EDUCATION LEVEL';
		loadFormContent('management/_educationFormModal.php', 'generalContent', 'lg', 'management/save-education', modalTitle, data, 'offcanvas');
	}

	async function updateRecord(id) {
		const res = await callApi('get', 'management/show-education/' + id);
		...
		Swal.fire({
			...
			async (result) => {
				if (result.isConfirmed) {
					loading('#bodyDiv', true);
					const res = await deleteApi(id, 'management/delete-education', getDataList);
					loading('#bodyDiv', false);
