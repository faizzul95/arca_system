...
<script>
	function loadFileContent(fileName, idToLoad, sizeModal = 'lg', title = 'Default Title', dataArray = null, typeModal = 'modal') {
		...
		return $.ajax({
			type: "POST",
			url: urls('public/custom/php/general.php'),
			data: {
				baseUrl: $('meta[name="base_url"]').attr('content'),
				fileName: fileName,
				dataArray: dataArray,
				// _token: Cookies.get(csrf_cookie_name)
				'arcaSecurityCsrf': Cookies.get(csrf_cookie_name) // csrf token
			},
			headers: {
	...
	function generateDatatable(id, typeTable = 'client', url = null, nodatadiv = 'nodatadiv', dataObj = null, filterColumn = []) {
		...
		if (typeTable == 'client') {
			...
			// 			}
			// 		}
			// 		dataArr['arcaSecurityCsrf'] = Cookies.get(csrf_cookie_name) // csrf token
			// 		dataSent = dataArr;
			...
			if (dataObj != null) {
				dataObj['arcaSecurityCsrf'] = Cookies.get(csrf_cookie_name) // csrf token
				dataSent = dataObj;
			} else {
				dataSent = null;
			}
