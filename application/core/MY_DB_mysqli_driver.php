...
class MY_DB_mysqli_driver extends CI_DB_mysqli_driver
{
    ...
    */
	...
	public function insert($table = '', $set = NULL, $escape = NULL)
	{
		...
		return ['status' => $status, 'lastID' => $lastID];
	}
	...
	public function trail($status, $event, $table, $set = NULL, $previous_values = NULL)
	{
		...
		if (isset($set['created_at'])) unset($set['created_at']); // remove created_at if exist

		if ($event == 'update') {
			if (isset($set['updated_at'])) unset($set['updated_at']); // remove updated_at if exist
		...
		if (isTableExist($this->CI->config->item('audit_table_name'))) {
			Logs(
				$event,
				[
					'old_value' => $old_value,
			...
			return parent::insert(
				...
				[
					...
					'role_id' => (isLoginCheck()) ? currentUserProfileID() : 0,
